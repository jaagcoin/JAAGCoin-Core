...
const char * const BITCOIN_CONF_FILENAME = "jaag.conf";
const char * const BITCOIN_PID_FILENAME = "jaagd.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "jaag" is a composite category enabling all Jaag-related debug output
                if(ptrCategory->count(std::string("jaag"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\JaagCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\JaagCore
    // Mac: ~/Library/Application Support/JaagCore
    // Unix: ~/.jaagcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "JaagCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/JaagCore";
#else
    // Unix
    return pathRet / ".jaagcore";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty jaag.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override jaag.conf
