...
*Setup instructions for a Gitian build of Jaag Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Jaag
...
- [Building Jaag Core](#building-jaag-core)
...
Clone the git repositories for Jaag Core and Gitian.
...
in the Jaag Core repository under 'Fetch and create inputs' to install sources which require
...
To build Jaag Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Jaag Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/jaag/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/jaagpay/jaag
...
For example:
```bash
URL=https://github.com/crowning-/jaag.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit jaag=${COMMIT} --url jaag=${URL} ../jaag/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit jaag=${COMMIT} --url jaag=${URL} ../jaag/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit jaag=${COMMIT} --url jaag=${URL} ../jaag/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the jaag git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../jaag/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/jaag
SIGPATH=/some/root/path/jaag-detached-sigs

./bin/gbuild --url jaag=${BTCPATH},signature=${SIGPATH} ../jaag/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/jaag-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/jaag-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/jaag-osx-build.assert
```
...
[jaag/gitian.sigs](https://github.com/jaagpay/gitian.sigs/) repository, or if that's not possible to create a pull
request.
