...
### Send 1000 JAAG to New Addresses

Send exactly 1000 JAAG to each new address created above.
...
Copy the masternode private key and correspondig collateral output transaction that holds the 1000 JAAG.

*Note: The masternode priviate key is **not** the same as a wallet private key. **Never** put your wallet private key in the masternode.conf file. That is almost equivalent to putting your 1000 JAAG on the remote server and defeats the purpose of a hot/cold setup.*
...
## Update jaag.conf on server

If you generated a new masternode private key, you will need to update the remote `jaag.conf` files.

Shut down the daemon and then edit the file.

```nano .jaagcore/jaag.conf```

### Edit the masternodeprivkey
If you generated a new masternode private key, you will need to update the `masternodeprivkey` value in your remote `jaag.conf` file.
...
```
jaag-cli masternode status
{
    "outpoint" : "<collateral_output>-<collateral_output_index>",
    "service" : "<ipaddress>:<port>",
    "pubkey" : "<1000 JAAG address>",
...
Search your Masternodes on https://jaagninja.pl/masternodes.html
